package in.gov.ntro.sih.sms.service;
import in.gov.ntro.sih.sms.domain.Capture;
import in.gov.ntro.sih.sms.repo.CaptureRepository;
import in.gov.ntro.sih.sms.repo.InvestigationRepository;
...
public class JobState {
    ...
    public Capture assign(Long id, Long investigationId) {
        if (investigationId != null && !investigations.existsById(investigationId))
            throw new in.gov.ntro.sih.sms.service.AnalysisService.UploadRejected("Investigation not found");
        Capture c = captures.lockById(id).orElseThrow(() -> new IllegalArgumentException("Run not found"));
        if (c.getStatus() != Capture.Status.PENDING) throw new in.gov.ntro.sih.sms.service.AnalysisService.UploadRejected("Only queued runs may be assigned");
    ...
    public Capture retry(Long id) {
        ...
        if (old.getStatus() == Capture.Status.RUNNING || old.getStatus() == Capture.Status.PENDING)
            throw new in.gov.ntro.sih.sms.service.AnalysisService.UploadRejected("Wait for the current run or cancel it first");
