package in.gov.ntro.sih.sms.web;

import com.fasterxml.jackson.databind.*;
import com.fasterxml.jackson.databind.node.*;
import in.gov.ntro.sih.sms.domain.*;
import in.gov.ntro.sih.sms.repo.*;
...
public class InvestigationController {
    ...
    @PostMapping public Investigation create(@RequestBody Map<String,String> body) {
        ...
        if (name.isEmpty() || name.length() > 160) throw new in.gov.ntro.sih.sms.service.AnalysisService.UploadRejected("Use an investigation name of 1–160 characters");
    ...
    public Map<String,Object> compare(@PathVariable Long id, @RequestParam Long before, @RequestParam Long after) {
        require(id);
        if (before.equals(after)) throw new in.gov.ntro.sih.sms.service.AnalysisService.UploadRejected("Select two different analysis runs");
    ...
    public Map<String,String> remediation(@PathVariable Long id, @RequestBody Map<String,String> body) {
        ...
        if (key.isBlank() || key.length() > 1000 || note.length() > 2000 || !Set.of("open", "in_progress", "fix_reported").contains(status))
            throw new in.gov.ntro.sih.sms.service.AnalysisService.UploadRejected("Invalid remediation status, key or note");
        ...
        if (!known) throw new in.gov.ntro.sih.sms.service.AnalysisService.UploadRejected("Finding does not belong to this investigation");
    ...
    private Capture run(Long id, Long captureId) {
        ...
        if (!Objects.equals(id,c.getInvestigationId()) || c.getStatus() != Capture.Status.COMPLETED)
            throw new in.gov.ntro.sih.sms.service.AnalysisService.UploadRejected("Comparison requires completed runs from this investigation");
        return c;
    }
    private JsonNode report(Capture c) {
        try { return mapper.readTree(c.getReportJson()); }
        catch (Exception e) { throw new in.gov.ntro.sih.sms.service.AnalysisService.UploadRejected("Stored report cannot be read"); }
    }
