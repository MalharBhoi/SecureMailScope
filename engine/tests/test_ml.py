...
from securemailscope.features import CORE_FEATURE_COUNT, FEATURE_NAMES, MISSING, extract
from securemailscope.ml.dataset import ServerProfile, generate, posture_of, sample_profile
from securemailscope.models import CertVisibility
...
def test_labels_never_come_from_the_rule_engine():
    ...
    from securemailscope.ml import dataset
...
def test_model_never_modifies_findings_or_grade(analysed, tmp_path):
    ...
    byte-identical.
    """
    from securemailscope.ml.predict import ModelUnavailable, annotate
...
def test_rounded_probabilities_still_sum_to_one():
    ...
    """
    from securemailscope.ml.predict import _round_to_one
...
def _doctored_artifact(tmp_path):
    ...
    from securemailscope.ml.train import DEFAULT_ARTIFACT_DIR
...
def test_model_records_the_sklearn_it_was_trained_with():
    ...
    """
    from securemailscope.ml.predict import load_model, ModelUnavailable
...
def test_version_mismatch_is_refused_not_silently_used(tmp_path):
    from securemailscope.ml.predict import (
        ModelUnavailable,
    ...
    assert "securemailscope.ml.train" in str(caught.value)
...
def test_analysis_survives_an_unusable_model(captures, tmp_path):
    ...
    assert "securemailscope.ml.train" in skipped[0], skipped[0]
