...
from securemailscope.models import CertVisibility, Protocol, Role, TlsMode
...
def test_imap_cleartext(analysed):
    ...
    assert "SMS-PROTO-003" in fired         # never encrypted on an access port
    assert "SMS-STRIP-004" in fired         # server never advertised STARTTLS
    assert "SMS-STRIP-001" not in fired, "LITERAL+ must not read as a stripped keyword"
...
def test_smtp_relay_client_declined_starttls(analysed):
    ...
    fired = {f.rule_id for f in s.findings}
    assert "SMS-STRIP-002" in fired         # offered but unused
    assert "SMS-AUTH-001" in fired
    assert "SMS-PROTO-004" in fired         # relay cleartext: high, not critical
    assert "SMS-PROTO-003" not in fired, "relay must not be graded as submission"
...
def test_relay_and_submission_are_graded_differently(analysed):
    ...
    assert relay_rules.get("SMS-PROTO-004") == "high"
    assert sub_rules.get("SMS-PROTO-003") == "critical"
...
def test_tls13_key_share_extension_parsing():
    ...
    from securemailscope.analyze.tls import parse_server_hello
...
def test_knowledge_base_is_a_real_package_not_a_namespace_one():
    """
    `securemailscope/kb/` must contain an `__init__.py`.
    ...
    spec = importlib.util.find_spec("securemailscope.kb")
    assert spec is not None, "the knowledge base package is missing entirely"
    assert spec.origin is not None, (
        "securemailscope.kb is a namespace package; add an __init__.py. "
...
def test_knowledge_base_files_load_without_importlib_resources():
    ...
    from securemailscope.analyze.ciphers import properties
    from securemailscope.kb import CIPHER_SUITES_FILE, RULES_FILE, kb_path
...
def test_a_missing_knowledge_base_says_so():
    ...
    from securemailscope.kb import kb_path
