...
_SMTP_CMD = re.compile(
    rb"^\s*(EHLO|HELO|MAIL FROM|RCPT TO|DATA|STARTTLS|AUTH|QUIT|RSET|NOOP|VRFY)\b",
    re.IGNORECASE,
)
_IMAP_CMD = re.compile(
    rb"^[A-Za-z0-9.]+\s+(CAPABILITY|LOGIN|AUTHENTICATE|SELECT|LIST|FETCH|"
    rb"STARTTLS|LOGOUT|NOOP|EXAMINE|STATUS|UID)\b",
    re.IGNORECASE,
)
_POP_CMD = re.compile(
    rb"^\s*(USER|PASS|APOP|CAPA|STLS|STAT|LIST|RETR|DELE|QUIT|TOP|UIDL)\b",
    re.IGNORECASE,
)
